...
These read as one-off bugs in `test-cases/` and `automation/`, but each is a specific instance of a risk
class that shows up more than once in this codebase. What I'd escalate to engineering leadership is the
class, not the instance — a fix for one checkout endpoint doesn't fix the pattern.
...
**2. Error handling at service boundaries is absent, not just incomplete.** Wherever a frontend/BFF route
...
**3. Behavioural contracts between services are undocumented — only the wire shape is.** Whether a
...
**4. Duplicated, independently-computed business logic across pages.** The same value — a converted
